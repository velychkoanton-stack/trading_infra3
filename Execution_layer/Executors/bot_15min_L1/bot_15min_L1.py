...
def build_config() -> ExecutorBotConfig:
    ...
    return ExecutorBotConfig(
        bot_id=_rule_str(rules, "bot_id", "bot_15min_L1"),
        environment=_rule_str(rules, "environment", "demo"),
        mysql_api_file=_rule_str(
            rules, "mysql_api_file", "api_mysql_main.txt"
        ),
        bybit_api_file=_rule_str(
            rules, "bybit_api_file", "api_credentials_demo_15min.txt"
        ),
        telegram_api_file=_rule_str(
            rules, "telegram_api_file", "api_telegram_level_1.txt"
        ),
        rules_file_path=str(rules_path),
        support_monitor=_rule_bool(rules, "support_monitor", False),
        support_monitor_interval_sec=_rule_int(
            rules, "support_monitor_interval_sec", 10
        ),
        heartbeat_interval_sec=_rule_int(
            rules, "heartbeat_interval_sec", 30
        ),
        worker_loop_sec=_rule_int(rules, "worker_loop_sec", 30),
        executor_threads_count=_rule_int(
            rules, "executor_threads_count", 4
        ),
        executor_start_delay_sec=_rule_float(
            rules, "executor_start_delay_sec", 20.0
        ),
        ws_stale_close_sec=_rule_int(rules, "ws_stale_close_sec", 300),
        signal_stale_sec=_rule_int(rules, "signal_stale_sec", 1200),
        pair_state_stale_sec=_rule_int(
            rules, "pair_state_stale_sec", 5400
        ),
        sql_dir_name=_rule_str(
            rules, "sql_dir_name", "sql_queries_15min"
        ),
        asset_lock_table=_rule_str(
            rules, "asset_lock_table", "asset_locks_15min"
        ),
        pair_state_timeframe_minutes=_rule_int(
            rules, "pair_state_timeframe_minutes", 60
        ),
    )
